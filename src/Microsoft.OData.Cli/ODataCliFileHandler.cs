...
namespace Microsoft.OData.Cli
{
    ...
    public class ODataCliFileHandler : IFileHandler
    {
        ...
        public void SetFileAsEmbeddedResource(string fileName)
        {
            if (this.project != null)
            {
                const string embeddedResourceTag = "EmbeddedResource";

                this.project = ProjectHelper.ReloadProject(this.project.DirectoryPath);
                if (!this.project.GetItems(embeddedResourceTag).Any(pi => pi.EvaluatedInclude.Equals(fileName)))
                {
                    ProjectHelper.AddProjectItem(this.project, embeddedResourceTag, fileName);
                }
            }
        }
    }
