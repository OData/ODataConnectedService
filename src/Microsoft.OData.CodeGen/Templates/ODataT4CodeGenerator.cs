...
private void ApplyParametersFromConfigurationClass()
{
    ...
    this.OmitVersioningInfo = Configuration.OmitVersioningInfo;
...
public abstract class ODataClientTemplate : TemplateBase
{
    ...
    /// <summary>
    /// Regex to match a string that begins with a numeric character, used to prefix leading numbers in valid identifiers
    /// </summary>
    private static readonly Regex RegexMatchNumericStart = new Regex(@"^\d.*$", RegexOptions.Compiled);

    /// <summary>
    /// Regex match any non-identifier characters, used to generate valid identifiers
    /// </summary>
    private static readonly Regex RegexMatchNonIdentifier = new Regex("[^A-Za-z0-9_]+", RegexOptions.Compiled);

    /// <summary>
    /// Default replacement character used to replace non-alphanumeric characters in valid identifiers
    /// </summary>
    ...
    private static string GetValidIdentifier(string name)
    {
        ...
        {
            // Spaces are not permitted, capitalize and strip spaces
            if (validName.Contains(" "))
            {
                validName = CultureInfo.CurrentCulture.TextInfo.ToTitleCase(validName);
                validName = validName.Replace(" ", "");
            }

            // Replace non-alphanumeric characters
            validName = RegexMatchNonIdentifier.Replace(validName, DefaultReplacementCharacter);

            // Prefix initial numbers with an underscore to meet requirements
            if (RegexMatchNumericStart.IsMatch(validName))
            {
                validName = $"{DefaultReplacementCharacter}{validName}";
            }
...
this.Write("//------------------------------------------------------------------------------\r" +
    ...
    if (!this.context.OmitVersioningInfo)
    {

this.Write("//     Runtime Version:");
